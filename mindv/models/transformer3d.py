...
class CogVideoXPatchEmbed(nn.Module):
    ...
    def forward(self, text_embeds: torch.Tensor, image_embeds: torch.Tensor, flow_embeds: Optional[torch.Tensor] = None):
        ...
        # 修复批次不匹配问题：如果text_embeds和image_embeds的批次大小不同，扩展text_embeds
        if text_batch_size != batch_size:
            if text_batch_size == 1 and batch_size > 1:
                # 如果text_embeds批次为1，image_embeds批次更大，复制text_embeds
                text_embeds = text_embeds.expand(batch_size, -1, -1)
                text_batch_size = batch_size
                # print(f"扩展text_embeds从批次1到{batch_size}以匹配image_embeds")
            elif batch_size == 1 and text_batch_size > 1:
                # 如果image_embeds批次为1，text_embeds批次更大，截断text_embeds
                text_embeds = text_embeds[:1]
                text_batch_size = 1
                # print(f"截断text_embeds从批次{text_batch_size}到1以匹配image_embeds")
            else:
                # 其他情况，使用最小的批次大小
                min_batch_size = min(text_batch_size, batch_size)
                text_embeds = text_embeds[:min_batch_size]
                image_embeds = image_embeds[:min_batch_size]
                batch_size = text_batch_size = min_batch_size
                # print(f"调整批次大小到最小值{min_batch_size}")
