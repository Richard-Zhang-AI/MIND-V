...
# 全局缓存用于存储已计算的image_to_video_latent
_image_latent_cache = {}

def clear_image_latent_cache():
    """清理image_latent缓存"""
    global _image_latent_cache
    _image_latent_cache.clear()
    print("image_latent缓存已清理")
...
def get_image_to_video_latent(validation_image_start, validation_image_end, video_length, sample_size):
    ...
    # 检查缓存
    if cache_key in _image_latent_cache:
        cached_result = _image_latent_cache[cache_key]
        # 返回缓存结果的副本，避免意外修改
        return cached_result[0].clone(), cached_result[1].clone(), cached_result[2]
    ...
    # 缓存结果
    _image_latent_cache[cache_key] = (input_video.clone(), input_video_mask.clone(), clip_image)
