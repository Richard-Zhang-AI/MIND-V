        ...
        if mask_video is not None:
            if (mask_video == 255).all():
                ...
                if num_channels_transformer != num_channels_latents:
                    ...
                    # FIX: Ensure mask_latents has the same number of frames as main latents
                    expected_frames = latents.shape[2]
                    if mask_latents.shape[2] != expected_frames:
                        mask_latents = mask_latents[:, :, :expected_frames, :, :]
                    if masked_video_latents.shape[2] != expected_frames:
                        masked_video_latents = masked_video_latents[:, :, :expected_frames, :, :]
